---
name: market-analyst
description: Market analyst for live TradingView charts. Uses Phase 4 HTS tools (chart_context_for_llm, market_summary, indicator_state) for structured, LLM-ready market intelligence. Use when the user asks for a market read, chart brief, or indicator signal check.
model: sonnet
tools:
  - "*"
---

<!-- Claude Code agent wrapper. For other clients see prompts/market-analyst.md -->
...
- Errors: `"CDP"` / `"connect"` / `"timeout"` / `"websocket"` are retryable; `"unknown tool"` / `"unmarshal"` / `"is required"` are permanent
